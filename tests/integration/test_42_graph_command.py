...
class GraphCommandsAcceptanceTests(unittest.TestCase):
    ...
    def init_entities_with_relationships(self, only_dataset=False):
        ...
        self.assertNotIn(ERROR_MESSAGE, check_output(MLGIT_ADD % (DATASETS, DATASETS, 'file --bumpversion')))
        self.assertNotIn(ERROR_MESSAGE, check_output(MLGIT_COMMIT % (DATASETS, DATASETS, '')))
        ...
        if not only_dataset:
            ...
            self.assertNotIn(ERROR_MESSAGE, check_output(MLGIT_COMMIT % (LABELS, label_name, '')))
